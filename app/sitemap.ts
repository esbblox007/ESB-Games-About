...
const STATIC_CONTENT_LAST_REVIEWED = new Date("2026-08-23T21:35:00Z");

export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const base = process.env.NEXT_PUBLIC_SITE_URL || "https://about.esbgames.com";
  const paths = ["", "/about", "/developer-hub", "/parental-controls", "/news", "/documentation", "/download", "/careers", "/careers/privacy", "/support", "/help", "/trust", "/trust/safety", "/subscriptions"];
  ...
  const helpEntries: MetadataRoute.Sitemap = helpCategories.map((category) => ({ url: `${base}/help/${category.id}`, lastModified: STATIC_CONTENT_LAST_REVIEWED, changeFrequency: "monthly", priority: 0.68 }));
  ...
  return [...staticEntries, ...policyEntries, ...articleEntries, ...starterDocumentationEntries, ...documentationEntries, ...supportEntries, ...helpEntries, ...careerEntries];
}
