...
const STATIC_CONTENT_LAST_REVIEWED = new Date("2026-08-23T15:30:00Z");
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const documentationEntries: MetadataRoute.Sitemap = documentation.map((article) => ({ url: `${base}/documentation/${article.slug}`, lastModified: new Date(article.updatedAt || article.publishedAt), changeFrequency: "monthly", priority: article.featured ? 0.8 : 0.7 }));
