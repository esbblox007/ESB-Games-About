...
export default async function DocumentationArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const related = relatedResult.articles.filter((item) => item.slug !== article.slug).slice(0, 3);
  ...
  return <PageShell><article className="news-article-page documentation-article-page">
    ...
    {article.tags.length > 0 && <div className="article-container article-tags" aria-label="Article tags">{article.tags.filter((tag) => tag.toLowerCase() !== "documentation-only").map((tag) => <span key={tag}>{tag}</span>)}</div>}
