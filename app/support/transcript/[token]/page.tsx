...
type Ticket = { id: string; ticket_reference: string; requester_name: string; requester_email: string | null; subject: string; category_id: string; team: string; status: string; priority: string; created_at: string; closed_at: string | null };
...
export default async function SupportTranscriptPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const [tickets, messages, attachments] = await Promise.all([
    supabaseSelect<Ticket>("support_tickets", `select=id,ticket_reference,requester_name,requester_email,subject,category_id,team,status,priority,created_at,closed_at&id=eq.${encodeURIComponent(record.ticket_id)}&limit=1`),
    ...
    supabaseSelect<Attachment>("support_ticket_attachments", `select=id,message_id,original_file_name,mime_type,size_bytes,scan_state,archived_at&ticket_id=eq.${encodeURIComponent(record.ticket_id)}&archived_at=is.null&order=created_at.asc`),
  ]);
  ...
  return <PageShell><main className="support-transcript-page"><div className="support-transcript-shell">
    ...
    <section className="support-transcript-heading"><div><span className="eyebrow">{ticket.ticket_reference}</span><h1>{ticket.subject}</h1><p>{categoryLabel(ticket.category_id)} · {ticket.team}</p></div><div><span>{priorityLabel(ticket.priority)} priority</span><strong>{ticket.status}</strong></div></section>
...
function priorityLabel(value: string) { return value === "Normal" ? "Medium" : value; }
