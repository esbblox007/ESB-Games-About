...
export default function SupportTicketClient({ accessToken }: { accessToken: string }) {
  ...
  return <section className="support-case-page support-ticket-app-page">
    <div className="container support-customer-workspace support-customer-app">
      <header className="support-customer-commandbar"><div className="support-case-brand"><span>ESB</span><div><small>ESB GAMES SUPPORT</small><strong>Private customer case</strong></div></div><div className="support-case-command-actions"><span className="support-secure-state"><i/> Secure session</span><a href="/support">Support centre</a></div></header>
      <div className="support-customer-case-header support-customer-compact-header"><div><span className="eyebrow">{data.ticket.reference}</span><h1>{data.ticket.subject}</h1><p>{categoryLabel(data.ticket.categoryId)} · Opened {formatDate(data.ticket.createdAt)}</p></div><div className="support-customer-badges"><span className="status">{data.ticket.status}</span></div></div>
      <div className="support-customer-compact-security"><span>✓</span><p><strong>Secure support conversation.</strong> Never share passwords, full payment-card details or verification codes.</p></div>
      <section className="support-customer-conversation support-customer-app-conversation" aria-label="Support conversation">
